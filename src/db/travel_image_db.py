...
def delete_travel_thumbnail_image(db : DatabaseHandler, place_id : int):
    query = """
        DELETE FROM travel_image
        WHERE place_id = %s AND is_thumbnail = TRUE
    """
    db.cursor.execute(query, (place_id,))


def delete_travel_detail_images(db : DatabaseHandler, place_id : int):
    query = """
        DELETE FROM travel_image
        WHERE place_id = %s AND is_thumbnail = FALSE
    """
